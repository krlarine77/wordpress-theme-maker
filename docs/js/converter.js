// converter.js — src/converter.js のブラウザ移植版
// require() / module.exports なし。グローバル関数として定義。
// cheerio の代わりに regex ベースの文字列操作を使用。
...
function convertHtmlToWordPress(htmlContent, filename) {
  ...
  // ── (1) ローカル CSS <link> タグを WordPress コメントに置換 ──────────────
  htmlContent = htmlContent.replace(/<link\b([^>]*)>/gi, (match) => {
    if (!/rel=["']stylesheet["']/i.test(match)) return match;
    const hrefM = match.match(/href=["']([^"']+)["']/i);
    if (!hrefM || hrefM[1].startsWith('http') || hrefM[1].startsWith('//')) return match;
    const href = hrefM[1];
    return (
  ...
  // ── (2) ローカル JS <script src> タグを WordPress コメントに置換 ──────────
  htmlContent = htmlContent.replace(/<script\b([^>]*)\s*>\s*<\/script>/gi, (match, attrs) => {
    const srcM = attrs.match(/src=["']([^"']+)["']/i);
    if (!srcM || srcM[1].startsWith('http') || srcM[1].startsWith('//')) return match;
    const src = srcM[1];
    return (
  ...
  // ── (3) ヘッダー nav のアンカーリンクにプレースホルダーを設定 ─────────────
  // href="#works" などのページ内アンカーをWordPress対応URLに変換する準備
  htmlContent = htmlContent.replace(/(<header\b[^>]*>)([\s\S]*?)(<\/header>)/i, (m, open, inner, close) => {
    // nav a[href^="#"] のアンカーをプレースホルダーに
    let out = inner.replace(
      /(<a\b[^>]*\s)href=["'](#)([^"']+)(["'])/gi,
      (am, before, hash, anchor, quote) => {
        if (anchor.length === 0) return am;
        return `${before}href=${quote}__WP_ANCHOR__${anchor}__${quote}`;
      }
    );
    // is-current クラスを除去
    out = out.replace(/\s+is-current\b/g, '');
    return open + out + close;
  });

  // ── (4) Contact Form: メール＋テキストエリアを持つフォームを CF7 プレースホルダーに変換 ──
  const CF7_MARKER = '___CF7_FORM_PLACEHOLDER___';
  htmlContent = htmlContent.replace(/<form\b[\s\S]*?<\/form>/gi, formHtml => {
    if (/type=["']email["']/i.test(formHtml) && /<textarea\b/i.test(formHtml)) {
      return CF7_MARKER;
    }
    return formHtml;
  });
  ...
  htmlContent = htmlContent.replace(
    /<div\b[^>]*class=["'][^"']*form[-_]success[^"']*["'][^>]*>[\s\S]*?<\/div>/gi,
    ''
  );
